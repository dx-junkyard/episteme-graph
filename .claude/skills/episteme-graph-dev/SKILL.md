...
| `backend/api/routes/learning.py` | 学習ルーター | コース管理・RAGチャット（intent_mode: on_path/explore/casual/**discuss**（論文と話す。予約疑似トピック `_discussion`・`discuss_scope` 2段スコープ・無断フォールバック禁止 DM1）・進捗・tension 確定 API・structure_anchor 確定 API（`/api/learning/anchors/...`）・音声会話（`/voice/transcribe`=Whisper STT, `/voice/speak`=TTS）・discuss 開幕画面（`GET .../discuss/opening`、非LLM読み取り専用）。RAG の全域検索は `search_chunks_with_metadata(..., allowed_document_ids=...)`（必須キーワード引数）で可視性を強制 — 可視集合の正本は `services.list_visible_document_ids` / コース sources 解決は `services.list_course_source_document_ids` |
...
| `backend/api/routes/discuss_observation.py` | discuss 観測基盤ルーター（migration 060 `discuss_metric_events`） | `admin_router`（SYSTEM_ADMIN のみ）: `GET /api/admin/discuss/observation-status`（蓄積状況 + 分析開始の参考目安）/ `GET /api/admin/discuss/observation-dump?format=tar.gz\|zip`（本文非含有・仮名化ダンプ。取得は `theory_review_events` `entity_type='discuss_observation'` に監査）。`learning_router`: `POST /api/learning/discuss/metric-events`（UI イベント取込。イベント語彙13 + payload キーのホワイトリスト、fire-and-forget 前提）。main.py にフラット登録（Tier 3-17c） |
