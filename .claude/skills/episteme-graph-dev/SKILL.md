...
| `backend/core/discuss/` | discuss モード（論文と話す）コア | `opening.py` = 開幕画面投影（thesis_reconstruction artifact + theory_component_graphs main 層 + open-assumptions。非LLM・読み取り専用・confidence 生数値非漏洩）。正本は `docs/features/discussion_mode_design.md`（DM1〜DM8）。**FastAPI を import しない** |
