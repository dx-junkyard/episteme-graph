...
| `backend/api/routes/learning.py` | 学習ルーター | コース管理・RAGチャット（intent_mode: on_path/explore/casual/**discuss**（論文と話す。予約疑似トピック `_discussion`・`discuss_scope` 2段スコープ・無断フォールバック禁止 DM1）・進捗・tension 確定 API・structure_anchor 確定 API（`/api/learning/anchors/...`）・音声会話（`/voice/transcribe`=Whisper STT, `/voice/speak`=TTS）・discuss 開幕画面（`GET .../discuss/opening`、非LLM読み取り専用）。RAG の全域検索は `search_chunks_with_metadata(..., allowed_document_ids=...)`（必須キーワード引数）で可視性を強制 — 可視集合の正本は `services.list_visible_document_ids` / コース sources 解決は `services.list_course_source_document_ids`。チャンク直読み `GET .../source-chunk/{chunk_id}` は **コース sources スコープ**で fail-closed（P0, 2026-08-11: `get_accessible_course_data` → `list_course_source_document_ids` → `get_chunk_passage(..., allowed_document_ids=)` 必須kw引数。全域可視集合に戻さない）、claim-refs は「コース sources ∪ 本人可視」の複合判定 |
...
| `backend/core/url_fetch.py` | URL教材取得の許可リスト + SSRF ガード付きダウンローダ | FastAPI 非 import。ドット境界のドメイン照合・`getaddrinfo` 全アドレスの private/loopback/link-local/reserved 検査・リダイレクト手動追跡（最大5ホップ・各ホップ再検証）・実バイトのマジック判定（拡張子/Content-Type を信用しない）・100MB/60秒上限。`fetch_source_from_url` は `allowed_domains` 必須引数で空は専用エラー（照合を迂回する公開関数を作らない）。許可リストは migration 070 `url_fetch_domains`（**シードしない** = 初期状態は空で機能無効） |
