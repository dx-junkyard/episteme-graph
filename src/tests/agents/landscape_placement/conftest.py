"""LandscapePlacementAgent のテスト用パス設定。

``LandscapePlacementAgent`` は修復ループを ``core/llm_worker/repair.py``
（9系統目のアダプタ・コピー実装しない方針）に委譲するため、``run()`` を実地で
動かすテストでは ``backend/`` が sys.path に必要になる。``backend/tests/conftest.py``
が逆方向（backend テストから ``src/`` を載せる）で同じことをしているのと対称の措置で、
``discuss_opening`` のテスト用 conftest と同型。
