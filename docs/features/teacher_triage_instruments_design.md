...
- レビュー是正（2026-08-15 レビュー後）:
  ①見通し判定で `daily_limit <= 0` のステージは「意図的に無効化」としてカウンタ集合から
  除外（見通しのトリガにしない。`remaining <= 0`＝枠はあるが枯渇のみ show の材料。
  全カウンタ無効化時は show=false — 上限 0 ステージによる恒久表示の是正）
  ②アップロードゾーンの見通し取得にも `?analyze_images=` を付与し、「画像も解析する」
  チェック変更時にイベント駆動で1回ずつ再取得（再解析モーダル側と同じ挙動・ポーリング禁止）
  ③WM 縮退の事実文はサーバ fact の一文（`lecture_wm.py WM_DEGRADED_NOTICE`）に一本化
  （JS 側の作文「記号の照合は表記の一致による近似です。」を削除 — 二重表示の是正。
  ui_static テストは「JS が縮退文を作文しない（サーバ fact 素通し）」の検査へ更新）。
- 運用注記: 複数コースに跨る document では説明キューの load ソートがキュー全件
  「影響度を導出できない候補」に縮退する（正直な縮退・仕様 — 実装判断⑤）。
