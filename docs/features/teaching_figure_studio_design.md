...
**ステータス: Phase 0〜2 実装済み（migration 063。§13 実装記録参照。※旧表記「設計（未実装）」は 2026-08-13 の総点検で更新）**
...
6. **[Minor] MinIO 再アップロードが DB 更新より前だった** — PATCH で先に配信物を上書きして
   ...
   **既知の残課題**: 学習者向け配信は MinIO のみを読む（`svg_source` フォールバックが無い）
   ...
   → **2026-08-01 解消**（解消注記 2026-09-10）。§13.2-7 のとおり同日の統合レーンで
   `routes/learning.py::get_course_figure_image` に同じフェイルソフトを適用済み（MinIO 取得
   失敗時は DB 正本の `svg_source` を配信し、空なら従来どおり 404）。**この項の「非スコープ /
   残課題」は現行実装には当たらない。**
