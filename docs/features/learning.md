...
- **いまここの周り（近傍関係ビュー）**: 正本は
  [personal_map_nearby_design.md](personal_map_nearby_design.md)（PMN-1〜PMN-7）。
  **地図が見せるのは周囲との関係**であって分類の配置ではない、という原則に沿って
  2 つの関係だけを描く — ①**縦軸＝依存の向き**（上＝これが前提にしていること／
  下＝これに依存していること。TheoryOperationGraph の main 層。`inferred` /
  `review_required` の推測辺は描かない）②**枠線＝確かめられているか**
  （`epistemic_ledger` の段階ラベル + 支持線の事実文。実線＝記帳あり／
  破線＝このコーパスの中では検証記録がない）。記号は本人の関与（問い／引っかかり／
  再構成）で、コースビューと同じ `.personal-map-dot-*` を共有する。
  「近く（前後1階層）」と「土台までの道筋」を切替でき、チップを押すと中心が移動する
  （同じ document の main 層内のみ）。台帳の記帳が1件も無ければ検証の区別ごと出さず、
  依存の向きだけを見せる（fail-closed）。座標・順位・距離・数値は描かない。
  普通のチャット質問（topic 縮退）は**範囲モード**でそのトピックが触れている論文の理論構成を
  粗く表示し、トピック⇄理論構成の対応が未記録のコースでも**コース範囲フォールバック**で
  ソース論文のバックボーンを「対応は未記録」の事実文付きで表示する（notice 1行で
  終わらせない。設計書 §11）。中心選択チップは発話の生テキストではなくトピック題名で表示される。
- **範囲モード（topic 縮退痕跡の事実ベース粗表示、2026-08-21）**: 普通のチャット質問など
  topic 粒度に縮退した痕跡（N3 等）も中心に選べる。`topics[].linked_claim_ids`
  （コース内容生成の決定論マッピング）→ claim → main 層ノードの**AI推定ゼロ**の経路で
  「このトピックの教材が触れている理論構成」を範囲として描く（`mode:"range"`、
  1点の中心を偽装しない）。触れているノードは通常表示・それ以外は淡色。topic 縮退痕跡は
  特定ノードに記録ドットとして**載せない**（偽精度の構造的禁止 — `node_matches_anchor` が
  topic に False を返す）。事実文で「どこについてかは、まだ記録されていません」と正直に言い、
  精密化の出口（テキスト選択質問 / 帰属カード confirm）を案内する。ノードを押すと従来の
  点ビュー（near/root）へ移動する。`linked_claim_ids` が無い・突合ゼロは
  `available:false` + 事実文に縮退。
- **広がり装置（2026-08-22）**: 正本は
  [personal_map_curiosity_design.md](personal_map_curiosity_design.md)。好奇心の文法＝
  「存在だけを事実として見せ、詳細は本人の明示操作まで伏せる」（cross_course_hint と
  同族）。①**名前のある霧** — 「いまの地図」タブの現在地の隣に、凍結骨格の隣接概念の
  名前だけを淡いチップで見せる（`GET /api/me/personal-network/atlas-neighbors`、
  非インタラクティブ・最大8件・失敗は何も描かない）②**共通部品の糸** — nearby 点ビューの
  中心ノードに confirmed 同一性リンク経由の「共通部品『◯』は、論文『△』にも現れます」
  （can_view フィルタ・最大3行）③**晴れ間の近接** — 表示集合外の未検証ノードを
  「この近くには、このコーパスの中では検証記録がない場所があります」の1行で提示
  ④**分野接続行** — 範囲ビューに「このトピックは、分野の地図の『◯』にある『△』に
  対応づけられています」。全装置とも非LLM・決定論・数値なし・失敗時はその行だけ静かに消える。
