...
> 2026-08-22 に UX 評価に基づく改修（§17 — 振り返りタブ削除・旅の縮退是正・タブ間導線・
> reconstruction ラベル）を追加実装**
...
- **新 migration なし**（v1。導出は既存テーブルの読みだけ。§5）。
  旅の同一性リンク区間のみ W-β（migration 048 同乗）に依存し、無ければ縮退（PN-7）。
...
- **atlas オーバーレイに「自分の記録を重ねる」トグル**を追加: 既存個人層（いまここ・
  足あと・霧）の上に、§2 のノードを kind 別ドット（問い / 引っかかり / 再構成 / 橋）で
  重ねる。骨格なし分野では領域ごと非表示（atlas fail-closed 継承）。ミニマップ（F-1）は
  **変更しない**（「いまここ + 状態ドット + 霧」のみの規約を維持）。
  （2026-07-17 N17 改名: 当初のトグルラベル「わたしの地図」は P-3 最上位パネル（§16）の
  パネル名と重複していたため「自分の記録を重ねる」へ改名。機能・API は不変。）
...
- **N4（再構成成功）の topic / atlas 解決は claim がトピック教材に組み込まれている
  場合のみ**（2026-07-17 N16 是正）。`derive.py` は `claim_id` を
  `topics[].linked_claim_ids`（`course_content_builder.py` が component 経由で書き込む
  既存の決定論マッピング）から逆引きし、解決できれば `topic_id` と
  `anchor.atlas_node_id` を設定する — これにより旅の [4]（atlas 骨格）/ [5]（近傍の
  本人ノード）区間と地図ドット配置が reconstruction ノードでも成立する。ただし
  **どのトピックの `linked_claim_ids` にも現れない claim**（コース内容生成を経ていない
  コース、agent mapping が component を紐づけなかったトピック、教材差し替え後に残った
  古い claim 等）は解決不能で、従来どおり `topic_id=None`・「まだ地図にない」トレイに
  残る（P4: 捨てない。旅は [4][5] を静かに省く）。reconstruction_items の由来 document
  → course sources → topic という代替経路は、document が複数トピックにまたがるため
  一意に決められず採用しなかった（誤ったトピック帰属はゼロ帰属より有害）。1つの claim
  が複数トピックに現れる場合は `iter_all_topics` の出現順で最初のトピックを採る（決定論）。
...
> **設計判断（2026-07-17 確定・調査 N34）: 兄弟検出のこの非対称は意図的仕様である。**
> コーススコープの旅（`journey_for_node`）は別コースの兄弟について
> `cross_course_hint`（「以前の学習につながる道があります」）**のみ**を返し、コース名・
> 件数・兄弟の詳細は本人が「以前の学習につながる道を見る」を押してコース横断版
> （`journey_for_person_node`）へ差し替えるまで伏せる。一方コース横断版の事実文は
> コース名を含める（「あなたが『◯◯』で残した問い…」— 出所を失わない）。
> 根拠: いま学んでいるコースの文脈に他コースの情報を**自動で**混ぜ込まない
> （自動で開かない・押し付けない原則）ことと、本人が明示的にコース外へ出た後は
> 出所（provenance）を正直に示すこと — の2原則の両立であり、実装の不揃いではない。
> 両者の挙動を「対称化」する変更は原則側の再検討なしに行わないこと。
...
- タブ構成（提案書 §3.2 A/C/D。B「このコースでの地図」は既存 personal-map.js の担当。
  当初は いまの地図 / 問いからの旅 / 振り返り の3タブ → 2026-08-18 に「いまここの周り」を
  先頭追加（[personal_map_nearby_design.md](personal_map_nearby_design.md)）→ **2026-08-22 に
  「振り返り」タブを削除**（§17。同一データの月別再掲で独自の情報価値が無く操作もゼロ、
  というオーナー裁定。現行は いまここの周り / いまの地図 / 問いからの旅 の3タブ）:
...
## §17 UX 評価に基づく改修（2026-08-22）

「いまここの周り」§11 が確立した原理（**縮退は例外ではなく主経路 / 粗い対応は隠すのではなく
粗いとラベルして見せる / 欠落は事実文＋出口で行き止まりにしない**）を残りタブへ水平展開した
改修。3体並列の調査（設計書要求 / フロント実装 / バックエンド実装・未露出データ棚卸し）で
確定した乖離のうち、設計判断が不要な是正一式と、オーナー裁定によるタブ削除を実施した。
migration 不要・LLM 不要・書き込み API 追加なし（PN-1〜7 の枠内）。

### 17.1 「振り返り」タブの削除（オーナー裁定）

- 本タブは設計上「月別グルーピング・件数/進捗率なし」の一文しか定義が無く、実装も
  `/api/me/personal-network` の結果の月別再掲のみで独自の情報価値・操作がゼロだった。
  時間軸の体験（動機・持ち越し・再訪）は設計上すでに**地図の外**にある
  （intention の時間層表示は understanding_cycle_design.md §12 で v1 非表示・
  carryover の再訪は「帰還の扉」・全痕跡の持ち出しは「わたしの記録」）ため、
  中途半端な第4タブを残すより削除が正直、という裁定（2026-08-22）。
- `renderReflect` と reflect タブ定義・専用スタイルを撤去。痕跡データ自体は不変
  （P4 — 表示面の削除であって記録の削除ではない）。

### 17.2 旅の縮退是正（journey.py）

- **空の旅を無言にしない**: `journey_for_person_node` は steps 空のとき `notice`
  （`NOTICE_JOURNEY_EMPTY`）+ `facts`（`nearby.FACT_RANGE_SHARPEN` を再利用 — 文言の
  重複表を作らない）を追加返却。フロントはカードを隠さず「この記録について」見出しで
  描画（従来は `area.hidden=true` で「押しても何も起きない」= 欠落の無言化だった）。
  steps 非空時はキー自体を足さない（後方互換）。
- **topic 縮退アンカーの範囲エントリ（[1']）**: topic アンカーの旅は従来 [1]〜[3] を丸ごと
  スキップし atlas binding 依存の [4][5] のみだった。§10（nearby 範囲モード）と同じ
  AI 推定ゼロの経路（`fetch_topic_claim_binding` → claim → main ノード、決定論順・
  fan-out ≤ 5・`can_view_document`）で「このトピックの教材が触れている理論構成」を
  [1'] 事実文 step として先頭に挿入する。前置き文で**トピック単位の対応であること**を
  明示し、1点の帰属を主張しない。**[2][3] は topic では実行しない**（粗い対応から
  confirmed 同一性リンクへ飛ぶと帰属の偽装になる）。[4][5]・step 総数 ≤ 12 は不変。
- コーススコープ版 `journey_for_node` は非改変（cross_course_hint の非対称も §16 の
  設計判断のまま）。

### 17.3 タブ間導線とラベルの是正

- **いまの地図 → いまここの周り**: ノード行に「この場所の周りを見る」
  （`data-pm-home-nearby-jump`）を追加。nearby の中心になれるアンカー（`NEARBY_ANCHOR_TYPES`
  と同一述語・関数共有で二重実装しない）を持つノードのみに出す。タブ名「いまの地図」と
  実体（リスト）の乖離を、図の担当タブへの引き渡しで埋める。
- **問いからの旅タブのノード行を `nodeRowHtml()` に統一**: 同じノードでもタブによって
  「地図には反映しない」の可否が変わる非対称を解消。
- **「別の問いから旅に出る」のノーオペ修正**: 旅タブ内では `switchTab` の early-return で
  何も起きなかったため、旅カードを閉じて起点リストへ戻す挙動に修正。
- **初期タブの不整合修正**: `invalidate()` の復帰タブを `open()` と同じ先頭タブ（nearby）に統一。
- **reconstruction ノードの label 改善**: 固定文字列「claim への再構成」をやめ、
  元 claim 本文の 80 字切り詰めを label にする（`queries.fetch_reconstructions_for_user` が
  claim 本文を join。対象行は本人が REVEAL を見た成功終端のみなので伏せフィールド漏洩に
  当たらない。claim 本文以外の item フィールドは引かない）。取得不能時は従来文字列へ
  フォールバック（捏造しない）。

### 17.4 残課題（本改修の非スコープ）

- kind の視覚記号のタブ間統一（nearby のドット記号体系の水平展開）— 磨き込み。
- 削除コース痕跡の空ラベルの中立表示。
- `derive_person_network` のコース単位 N+1（コース数増で線形悪化）。
- コーススコープ版 journey への topic 範囲エントリ適用。
- 供給側の根治（confirmed 同一性リンク・atlas binding の充実を教員へ促す G層 To-Do）—
  `course.topics_unlinked` と同列の別 issue。
