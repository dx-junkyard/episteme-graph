...
- 確定は 2 タップ（カード → 確定）。理由は必須で、**既定文をプリフィルしない**
  （2026-09-10 是正 F7a。当初案の「既定文候補から選択 + 追記可」は実装では固定文1つの
  プリフィルになり、Enter を押すだけで「検出根拠を確認し…」が本人の言葉として記帳される
  状態を作っていた。vision §4「人間のクリックを正当性と誤認しない」に抵触するため撤去。
  入力負荷の軽減は、既定文ではなく**過去の却下・確定理由の再利用**
  （`six_lenses_2026-09-10/02_teacher.md` 提案1）で扱う）
...
> **設計判断（2026-07-17 確定・調査 N40）: セッション一覧のノード数表示は数値のまま維持する。**
> 保存済み反実仮想セッション一覧（`doubt-atlas.js::loadCfSessions`）は
> 「この前提に依存する範囲: N ノード / 両者に依存（判定不能）: M ノード」と
> collapsed / indeterminate の**件数を素の数値で**表示している。これは D層の
> 数値非表示原則の違反ではない — 同原則が隠す対象は **`load_score` と `confidence`
> という評価・確度のスコア**（人・主張の格付けに転用され得る数値）であり、
> counterfactual のノード数は「この前提に依存する下流ノードがいくつあるか」という
> **理論構造の事実**（決定論的計算の結果、誰の評価でもない）である。事実の数を
> 段階ラベルに丸めるとかえって「何かを隠している」表示になるため、数値表示のまま
> 維持と決定した。`test_doubt_guardrails.py` の数値非表示検査の対象にも含めない。
