...
> **解消済み注記（2026-08-13）:** 本レビューの P1×2 / P2×2 の4指摘は、その後の実装で
> **すべて解消済み**であることをコード実読で確認した（`derive.py` は `connected_refs` のみ使用 /
> journey の可視性ゲート / retired ライブラリエントリの除外 / コース切替時の非同期競合対策）。
> 当時のレビュー記録として保存する — 未修正バグ一覧として読まないこと。
