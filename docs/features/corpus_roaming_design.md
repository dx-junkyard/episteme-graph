...
| **CR3** | **数値を見せない・地図は投影** | LS 系を継承: weight・confidence・件数・類似度を学習者に出さない。配置には出所ラベル（「AIによる推定（未確認）」「教員確認済み」）を必ず付ける。踏破率・カバー率を作らない。**（2026-08-28 オーナー裁定・解釈の追記）** 本条項の「数値を見せない」は**件数そのものを描かないこと**を指す。**段階ビン（離散の濃淡・サイズ）による表現は本条項に反しない** — 既存の段階ラベル（関連: 高/中/低）・レンジ表示（3-5 / 6-10 / 11+）と同じ流儀の視覚版という位置づけ。**連続比例（半径・濃度 ∝ 件数）と、件数・閾値そのものの表示は引き続き禁止**。原条項は取り消さない（適用対象の解釈を確定しただけ）。 |
...
- **密度表現（2026-08-28 追記・CR3 の裁定に基づく現行仕様）**: 論文1本＝1マーカーの羅列を
  やめ、**領域の塗り**（配置された distinct document 数の4段階ビン）と**アンカーごとの
  段階サイズの円1つ**（同じく段階ビン）で「閲覧できる論文の厚み」を示す。地図直下に段階
  スウォッチの凡例を置き、**件数・閾値・数値は一切描かない**。アンカーの円をタップすると
  右の論文リストがそのアンカーの論文に絞り込まれる（解除可）。導出はクライアント側で
  手元の placements と骨格から決定論的に行い、API・DB は変えない（詳細は §12）。
...
## 13. 実装記録（地図の密度表現「案A」, 2026-08-28）

Phase A の地図の読み取りやすさに対するオーナー裁定（同日）に基づき、論文1本＝1マーカーの
羅列（§12 学習UI の 📄 マーカー・溢れの「…」）を**段階ビンによる密度表現**に置き換えた。
**API・migration・バックエンドの変更なし**（`GET /api/learning/corpus/landscape` の
レスポンス形は不変）。密度は手元の placements（`anchor_node_id` / `document_id`）と
L1 骨格（`nodes[].region`）から**クライアント側で決定論導出**する（distinct document 数で
数え、同一論文の複数配置を重複計上しない）。

- **CR3 の解釈確定（§2 に追記済み）**: 「数値を見せない」は件数そのものを描かないことを
  指し、段階ビン（離散の濃淡・サイズ）は可。連続比例（半径・濃度 ∝ 件数）と件数・閾値の
  表示は引き続き禁止。既存の段階ラベル・レンジ表示の視覚版という位置づけ。
- **領域の塗り**: 領域に置かれた distinct document 数を 0 / 1–2 / 3–5 / 6+ の4段ビンに
  落とし、CSS クラス `d0`〜`d3` で塗りを段階的に濃くする。fog 領域（`kind="fog"`）は
  従来の霧表現のまま（密度で塗り分けない — 霧は「名前だけ」の文法を維持）。
- **アンカーの円**: 📄 マーカーの羅列（最大4件 +「…」）を廃止し、アンカー（概念ノード /
  領域）ごとに**段階サイズの円1つ**にする。1件→小 / 2–3件→中 / 4件以上→大の固定3段
  （r = 6 / 9 / 12。件数に比例させない）。円をクリックすると右の論文リストが**そのアンカー
  の論文に絞り込まれ**、解除ボタンで元に戻る（絞り込み中も件数は表示しない）。
- **凡例**: 地図直下に「閲覧できる論文の厚み」と 4 段スウォッチ（なし / 少しある /
  蓄積あり / 厚い）。**数値・閾値・件数は一切表示しない**（閾値は設計書・実装側にのみ存在
  する内部値）。
- **変更ファイル**: `frontend/public/js/corpus-sea.js` / `frontend/public/css/styles.css` /
  `backend/tests/test_corpus_roaming_ui_static.py`（凡例文言・段階クラス・比例描画と
  件数表示の不在を静的検査で固定）。CR5（バッジ・自動表示なし・ポーリング禁止）と
  CR1（可視論文のみが密度に寄与）は不変。
