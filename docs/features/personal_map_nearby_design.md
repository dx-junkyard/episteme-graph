...
> フォールバック + has_touch 撤廃 + 出口案内 + topic チップの題名表示）**、2026-08-23 に
> **ノード情報の拡充（§12、代表 claim 逐語 + 検証ラベルの差分表示化）**を追加実装。
> migration 不要（既存テーブルの読みのみ）。以後は §9 / §10.6 / §11.4 / §12 実装記録の追記のみ
...
（追記 2026-08-22: 「振り返り」タブはその後のオーナー裁定で削除され、現行は
いまここの周り / いまの地図 / 問いからの旅 の3タブ。§11 の原理の残りタブへの水平展開
（旅の縮退是正・タブ間導線）と併せ、記録は親文書 §17。）
...
## §12 ノード情報の拡充 — 代表 claim 逐語と検証ラベルの差分表示（2026-08-23）

### 12.1 動機 — 索引としては合格・地図としては未達

範囲ビュー（特に §11 のコース範囲フォールバック）の実データ精査（2026-08-23、オーナー
評価）で、チップに読み取れる情報が **stage 名と「検証情報なし」だけ**であることが判明した。
原因は2つの独立した事情の合成:

1. **ノードが「何を言っている場所か」を語らない**。main ノードの label は #308 で短い
   stage 名固定、長い `description` は内部 ID（`eq_2_7` 等）混入のため §9 で学習者 DTO
   から除外済み。§9 当時の根拠「場所の同定は stage 名 + 本人の痕跡テキストで足りる」は
   mine が構造的に空になるフォールバック範囲ビュー（§11、後から追加）では成立しない。
2. **「検証情報なし」の氾濫（設計と実装のミスマッチ）**。§6 は「台帳行ゼロ →
   `ledger_available:false` → 検証区別ごと非表示」の縮退を用意していたが、D層
   `ledger_builder` が main ノードへ `unknown` 状態の行を機械的にバックフィルするため
   「行ゼロ」が実データでほぼ発生せず、無情報ラベルが全チップに描かれていた。
   **一様なラベルは信号ではなくノイズ**であり、R2（確かめられているかで困り方の性質が
   変わる）の提供価値を毀損していた。

### 12.2 是正A — 代表 claim の逐語 `claim_excerpt`

- ノード DTO（点・範囲共通の `_node_dto`）に `claim_excerpt`（string | null）を追加。
  ノードの `linked_claim_ids` から **source_backed（教員承認済みを優先）** の claim 本文を
  claim_id 昇順の決定論で1件選び、80字切り詰め（derive の `_truncate` と同規則）。
  source_backed が無ければ null（未承認・非 source_backed の text を学習者に出さない）。
  AI 推定ゼロ・数値ゼロ・verbatim。claim 本文の学習者露出は claim チップ・再構成リビール
  と同クラスで新しい開示ではない。
- UI は**範囲ビューのチップ2行目にのみ**描画（横レーンの点ビューチップは幅が無いため
  描画しない — DTO は統一、描画面で絞る）。「理論の土台」（分類）に「『…』」（内容の
  逐語）が付くことで、初めてノードが本人の読んだ内容と対応づく。

### 12.3 是正B — 検証ラベルの差分表示化

- 表示対象ノードの台帳 status に**差分となる status**（`directly_verified` /
  `indirectly_supported` / `refuted` / `untested`）が1つも無い場合（全ノードが行なし or
  `unknown`）: 全ノード `verification: null` + `ledger_available: false` に倒し、facts に
  `FACT_NO_VERIFICATION_RECORDS`（「これらの理論構成には、このコーパスの中では検証記録が
  ありません。」— SL1 閉世界語彙）を1行追加する。既存の「台帳ゼロ」UI 経路（中立枠・
  凡例から検証項目を落とす）をそのまま再利用する。
- 差分 status が1つでもあれば従来どおり per-node 表示（**差分＝信号、一様＝ノイズ**）。
  `ledger_available` の意味論は「台帳行ゼロ、または表示ノードに差分となる検証情報が無い」
  へ拡張（PMN-3 の「記帳の有無を主語にする」意味論は不変 — 一様な不在を1つの事実文に
  集約しただけ）。点ビュー・範囲ビューの両方に適用。装置3「晴れ間の近接」は ledger dict を
  直接読むため本変更と独立（テストで固定）。

### 12.4 非スコープ（本節時点）

- エッジへの動詞ラベル付与（edge_type → 「定義する / 導出する」等。案C）— 後続の磨き込み。
- 点ビューチップへの excerpt 描画・`description` のサニタイズ復活。
