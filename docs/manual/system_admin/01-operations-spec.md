...
| ロール | できること |
|---|---|
| 受講者（STUDENT） | 学習UIの利用、RAGチャット、コース受講登録、グループ招待の承諾・招待コードでの参加 |
| 教員（TEACHER） | 受講者の権限に加えて、教材アップロード、コース作成・公開、グループ管理、理論コンポーネントの承認・共有（C層）、学生アカウント作成、学生アカウントの停止・再開、アカウント一覧の閲覧（学生に限定） |
| システム管理者（SYSTEM_ADMIN） | 教員の権限に加えて、教員アカウントの作成、教員アカウントへの操作、パスワード再設定（対象が学生でも教員でも）、個別アカウントの利用実績照会、削除予約と所有物の移管、運用系タブ（システム統計・エラー解析・LLM使用量・マニュアル編集・discuss観測・AIモデル）の利用 |

権限チェックはロール依存の関数（TEACHER 未満は 403 / SYSTEM_ADMIN 以外は 403）として実装されています。受講者アカウントは教員以上が管理UIから作成します。教員アカウントの作成はシステム管理者のみが行えます。初期のシステム管理者は起動時に環境変数 `ADMIN_PASSWORD` で作成されます。受講者が自分でアカウントを新規登録する画面はありません（ログイン画面にはログインフォームのみがあります）。

アカウントの停止・パスワード再設定・削除予約・所有物の移管といったライフサイクル運用の詳細は
[教員管理（管理画面）](10-admin-teachers.md) を参照してください。

### 管理UIのタブはロールで出し分けられます {#admin-tab-visibility}

管理UI（`/admin.html`）のタブ構成は、ロールによって変わります。

- **教員（TEACHER）**: 教材管理・コースビルダー・コース管理・原稿スタジオなどの制作系タブと、
  学生管理・グループ・分野の地図などの運用系タブが表示されます。
- **システム管理者（SYSTEM_ADMIN）**: 教員管理・システム統計・エラー解析・LLM使用量・
  マニュアル編集・discuss観測・AIモデルの各タブが追加で表示されます。**その一方で、
  教材管理・コースビルダー・コース管理・原稿スタジオのタブは表示されません**（ログイン直後は
  エラー解析タブが開きます）。これらの制作作業は教員アカウントで行ってください。
- **受講者（STUDENT）**: 管理UIを開いても学習UIへ戻されます。

なお、これは画面の出し分けであって、権限そのもの（API 側の判定）は上の表のとおり累積です。
