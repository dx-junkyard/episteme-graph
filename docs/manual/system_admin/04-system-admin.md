...
| `LEARNING_CHAT_STREAMING_ENABLED` | 学習チャットの回答を**書かれている途中から**受講者に見せるかどうか（既定 `false` = 完成してから一度に表示）。有効にすると受講者は生成中に「停止」で止められます（止めた往復は履歴にも問いの軌跡にも残りません）。AI の呼び出し回数・上限・回答の中身は変わりません |
