...
> **状態:** 生きたリファレンス（2026-09-18 新設・同日改訂 = 族/型の 2 段構成）。**族**（`###`）は原因の
> 性質の大づかみな括り、**型**（`####`）はその中の見分けのつく形で、エントリの `pattern` は型の slug を
> 参照する。**エントリの無い型は置かない**（ガードレールが弾く）。型の成立（確定エントリ 2 件以上）と
> 暫定の区別、各型に属するエントリの一覧、「主分類の傾向」とエントリの食い違いは [index.md](index.md)
> が機械生成する（ここに手で列挙しない）。規則は [taxonomy.md §6.1](taxonomy.md)。

各族の見出し直後に族の表（一般形・主 facet・含む型）、各型の見出し直後に型の表（一般形・主分類の
傾向・一般化レベル・典型的な発見観点・典型的な解決観点・見分け方）を書く。表の値は語彙
...
局所の型が少ないのは、`docs/` で管理対象になる課題が調査・レビュー由来で、単一処理の不良は
...
| 主分類の傾向 | `local`（`local.logic`）。照合器が複数箇所に散っているなら `structure.aggregation` を副分類に |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `local`（`local.resource`）。副分類に `governance.completion` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `local`（語彙表が分散しているなら `structure.aggregation` を副分類に） |
| 一般化レベル | `general` |
...
| 主な facet | `structure.representation` |
...
| 主分類の傾向 | `structure`（`structure.representation`）。副分類に `connection.version` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.representation`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.representation`）。副分類に `connection.target` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.representation`）。副分類に `connection.meaning` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.decomposition`） |
| 一般化レベル | `general` |
...
| 主な facet | `structure.aggregation` / `structure.responsibility` |
...
| 主分類の傾向 | `structure`（`structure.aggregation`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.responsibility` / `structure.representation`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.aggregation`）。副分類に `governance.review` |
| 一般化レベル | `repo_pattern` |
...
| 主分類の傾向 | `structure`（`structure.representation`）。副分類に `connection.information` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.representation`）。副分類に `governance.review` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.responsibility` / `structure.representation`）。副分類に `local.logic` |
| 一般化レベル | `general` |
...
| 主な facet | `structure.decomposition` / `structure.responsibility` |
...
| 主分類の傾向 | `structure`（`structure.responsibility`）。副分類に `connection.contract` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `structure`（`structure.decomposition`）。副分類に `governance.review` |
| 一般化レベル | `repo_pattern` |
...
| 主分類の傾向 | `structure`（`structure.responsibility`）。副分類に `governance.review` |
| 一般化レベル | `general` |
...
| 主な facet | `connection.condition` / `connection.target` |
...
| 主分類の傾向 | `connection`（`connection.condition`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `connection`（`connection.target`）。副分類に `connection.condition` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `connection`（`connection.target`）。副分類に `structure.aggregation` |
| 一般化レベル | `general` |
...
| 主な facet | `connection.contract` / `connection.meaning` |
...
| 主分類の傾向 | `connection`（`connection.contract`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `connection`（`connection.contract`）。副分類に `structure.representation` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `connection`（`connection.meaning`）。副分類に `local.logic` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `connection`（`connection.meaning` — データが指示として解釈される） |
| 一般化レベル | `general` |
...
| 主な facet | `connection.information` / `connection.version` |
...
| 主分類の傾向 | `connection`（`connection.version`）。副分類に `governance.completion` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `connection`（`connection.information`）。副分類に `structure.responsibility` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `connection`（`connection.information`）。副分類に `governance.completion` |
| 一般化レベル | `repo_pattern` |
...
| 主分類の傾向 | `connection`（`connection.information`）。副分類に `governance.resume` |
| 一般化レベル | `general` |
...
| 主な facet | `governance.assignment` / `governance.completion` |
...
| 主分類の傾向 | `governance`（`governance.assignment`） |
| 一般化レベル | `repo_pattern` |
...
| 主分類の傾向 | `governance`（`governance.assignment`）。副分類に `connection.condition` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `governance`（`governance.completion`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `governance`（`governance.resume`）。副分類に `connection.version` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `governance`（`governance.resume`）。副分類に `structure.representation` |
| 一般化レベル | `general` |
...
| 主な facet | `governance.ordering` / `governance.budget` |
...
| 主分類の傾向 | `governance`（`governance.ordering`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `governance`（`governance.budget`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `governance`（`governance.review`） |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `governance`（`governance.review` — 更新手続の欠落）。正本が複数なら `structure.aggregation` |
| 一般化レベル | `general` |
...
| 主分類の傾向 | `governance`（`governance.review`）。副分類に `structure.aggregation` |
| 一般化レベル | `general` |
