...
classification:
  ...
  axis_confidence:          # 軸ごとの確信度 high | medium | low（taxonomy §2.1。none にも付ける）
    processing: high
    structure: high
    connection: medium
    governance: low
  proposals: []             # 確信度 low / medium の軸から出た新設の提案（taxonomy §2.2）。無ければ []
...
- **確信度は軸ごと**。低い軸には「新しい値か軸が要るか」を問い、要るなら `proposals` に 1 件。要らなければ空。
