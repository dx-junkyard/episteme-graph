...
> **状態: 設計中**（3段の順序と依存を確定した計画文書。各段の正本設計書は §3 の表。
> 実装が始まったら各段の設計書側に §実装記録を足し、本書は §6 の判定表だけを更新する）
...
|---|---|---|---|
| 1 | 学習者が composer 1つで「軽い質問」「論文との議論」を書き分けずに済み、推定が外れたら1タップで直せる。casual にテキストから入れる | HELP pre-route の順序不変・DM1 のスコープ不変・discuss 中に casual を推定しない・cycle を推定で入らない・`stance_source` が痕跡に残る・LLM 回数 +0〜1 | 設計中 |
