...
**→ 2026-09-10 一部解消（第1波 #9 = 提案2 の最小形）**: 6軸を**宣言**する層を実装した
（`backend/core/disclosure_axes.py` + `GET /api/disclosure`（ロールゲートなし）+
各対話 UI の入力欄に常設の事実文1行 + `docs/manual/student/02-student.md` §18 /
`docs/manual/teacher/10-admin-common.md` の2節。正本は
[`disclosure_axes_design.md`](../../features/disclosure_axes_design.md)）。**第5軸の
「不告知」はこれで解消**し、外部 AI を通らない経路も同じ表に併記した。ただし解消したのは
**告知**であって、可視性の**選択肢**（第1軸の学習者側 = 提案1 / 第2軸の帰属と開示の分離 =
提案3 / 第3軸の条件付き再利用 / 第6軸の「止める・出る」）は未実装のまま。本人向けの
AI 通過点履歴（`GET /api/me/ai-touchpoints`）と初回告知カード（オーナー判断 D4）も未着手。
