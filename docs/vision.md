...
`core/decision_context.py` の `BASIS_*` 定数**（2026-09-10 時点は [リリース前の確認](features/release_review_flow_design.md)
§6 追補のステップ2「この配置で次へ」・説明レビューの一括承認／一括却下・学習マップの
対応付け保存の3経路。件数を他文書に書き写さず定数を数える）。影響機能の整理は
...
| **確定文脈の記帳の全面適用** | 2026-09-04 に一括確定へ導入した `decision_context` を、単発の承認・凍結・公開にも広げる | 段階適用中（§4）。適用済みの経路は `core/decision_context.py` の `BASIS_*` 定数が正本（2026-09-10 時点3経路 — リリース前の確認・説明レビュー一括・学習マップ対応付け保存）。ガードレールで「一括確定は必ず記帳」を固定済み |
