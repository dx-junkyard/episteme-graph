...
`theory_review_events` の payload に `decision_context` を積む。**適用先の正本は
`core/decision_context.py` の `BASIS_*` 定数**（[リリース前の確認](features/release_review_flow_design.md)
§6 追補のステップ2「この配置で次へ」・説明レビューの一括承認／一括却下・学習マップの
対応付け保存に加え、2026-09-10 に一括取り込み・骨格の凍結・コースの公開・単発の承認へ拡張。
件数・一覧を他文書に書き写さず定数を数える）。影響機能の整理は
...
| **自分で確かめた痕跡** | 再導出・桁の見積もり・図の再現という「世界との接触」を、答責を伴わない**確認行為**として本人の記録に残す（`interest_traces` の新 kind `verification_attempt` 案 — 登録簿 `core/trace_registry.py` への露出3宣言が前提）。学習者産出物のうち唯一「主張」ではなく「点検」なので、可視性の勾配を開くならここが最初の候補 | 優先度 高（[2026-09-03 討論](architecture/vision_future_debate_2026-09-03.md) §7.2・3グループ独立収束の最優先）。本人のみ可視（PN-1）を死守し、D層・SL層の学習者側の対称物として設計する。着手の合流先は [六つのレンズ調査](architecture/vision_ux_gap_six_lenses_2026-09-10.md) §8（A-20・第2波） |
...
| **確定文脈の記帳の全面適用** | 2026-09-04 に一括確定へ導入した `decision_context` を、残る確定（学習者側の確定など）にも広げる | 段階適用中（§4）。適用済みの経路は `core/decision_context.py` の `BASIS_*` 定数が正本（2026-09-10 に一括取り込み・骨格の凍結・コースの公開・単発の承認まで拡張。**残りは学習者側の確定**で、PN-1 / P3 との兼ね合いを先に決める）。ガードレールで「一括確定は必ず記帳」を固定済み |
