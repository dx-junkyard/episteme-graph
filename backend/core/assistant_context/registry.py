...
def resolve(
    ...
    *,
    ...
    ``kinds`` を渡すと、登録済みのうちその種別だけを**登録順のまま**適用する
    （Phase 4 §11.5: ``cycle_mode="elicit"`` は表示モードの事実1行だけを許し、
    主張本文・検証事実は渡さない = 問いの答えを手渡さない）。絞り込みは
    **route が明示した時だけ**効き、既定（``None``）は Phase 1 と同一挙動。
    """
    if ctx is None:
        return []
    allowed = None if kinds is None else {str(kind) for kind in kinds}
    payload: Mapping[str, Any] = sources if isinstance(sources, Mapping) else {}
    facts: list[str] = []
    for _kind, resolver in tuple(_REGISTRY.get(ctx.screen, ())):
        if allowed is not None and _kind not in allowed:
            continue
        try:
...
def render_block(
    ...
    *,
    ...
    空なら空文字。``max_chars`` を超える場合は**行境界**で打ち切り、
    最終行に ``…（以下省略）`` を置く（件数は書かない = SA4）。

    ``header`` / ``max_chars`` は additive kwarg で、既定値は Phase 1 と同じ
    （学習側は ``BLOCK_HEADER_LEARNING`` / ``MAX_BLOCK_CHARS_LEARNING`` を渡す。
    §11.3 — 教員向けヘッダをそのまま学習者に使うと文面が嘘になる）。
    ...
    budget = max_chars - len(header) - 1 - (len(TRUNCATION_LINE) + 1)
    kept: list[str] = []
    used = 0
    for line in lines:
        ...
        kept.append(line)
        used += need
    return "\n".join([header, *kept, TRUNCATION_LINE])
