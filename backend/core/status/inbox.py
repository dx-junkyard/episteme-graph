...
core/versioning/notifications.py（share_notifications）と同型。行削除しない（S4）。
"""
    ...
    try:
        result = session.execute(
            sa_text("""
                ...
                WHERE id = CAST(:nid AS uuid) AND recipient_id = CAST(:uid AS uuid) AND dismissed_at IS NULL
            """),
            {"nid": notification_id, "uid": recipient_id},
        )
        session.commit()
